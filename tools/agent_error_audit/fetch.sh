...
for r in $REPOS; do
  ( curl -sSL --fail --noproxy '*' -H "Authorization: Bearer $GITHUB_PAT" \
    ...
    if [ $code -ne 0 ]; then echo "$r FETCH_FAIL:$code"; else
      mkdir -p "corpus/$r"
      if ! tar -tzf "corpus/$r.tar.gz" >/dev/null 2>&1; then
        echo "$r TARBALL_INVALID — corpus incomplete, do not publish counts"; exit 1
      fi
