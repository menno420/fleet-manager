...
# Game Lab — failsafe cron text (registry copy, prompts v3.8)
...
- **cron:** `50 */2 * * *` — slot per the stagger table
...
this lane's heartbeats + fm telemetry/triggers-snapshot.json — this lane's ids only; plus ids the heartbeat marks left-for-successor. `list_triggers` is
