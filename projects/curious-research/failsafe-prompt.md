<!-- v1 · 2026-07-13 · fleet-manager projects registry — GENERATED COPY, do not edit
...
# Curious Research — failsafe cron text (registry copy, prompts v3.6)
...
> Version lineage: v1 (2026-07-13) supersedes the prior registry sync copy.
...
<!-- registry-header-end -->
# Curious Research — failsafe cron (dead-man wake, Q-0265)

- **Routine name:** `Curious Research failsafe wake`
- **cron:** `20 */2 * * *` — slot per the stagger table
...
```
FAILSAFE WAKE (Curious Research, Q-0265): send_later chain alive → verify in one line, end. Stalled → resume the work loop (sync HEAD → inbox → slice after slice, landed per LANDING), re-arm the chain (~15 min), and write your heartbeat (control/status.md, per-seat grammar) as the deliberate last step.
```
...
the predecessor heartbeat (control/status.md routine block); plus ids the heartbeat marks left-for-successor. `list_triggers` is
