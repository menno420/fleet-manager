...
# Self Improvement — failsafe cron text (registry copy, prompts v3.8)
...
- **cron:** `0 */2 * * *` — slot per the stagger table
...
(STATELESS, D-9) — find old ids in: the kit heartbeat's routine block; the kit-lab DAILY is an owner BUSINESS cron — KEEP (kill-switch: docs/operations/lab-loop.md); plus ids the heartbeat
