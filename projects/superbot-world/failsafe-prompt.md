...
# SuperBot World — failsafe cron text (registry copy, prompts v3.2)
...
- **cron:** `15 1-23/2 * * *` — slot per the v3.2 stagger table
...
the predecessor heartbeats + fleet-manager telemetry/triggers-snapshot.json — verify each ("maybe auto-disabled" ≠ "never armed"); plus ids the heartbeat marks left-for-successor. `list_triggers` is
