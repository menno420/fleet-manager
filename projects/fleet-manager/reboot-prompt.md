...
c. ONLY THEN **disable** the archived session's failsafe — `update_trigger` trig_01F9UdoUtLy8oknBPBkHLshS with `enabled: false`, then verify via `list_triggers` that it reports disabled.
   ⚠ **AMENDED 2026-08-09 — this step used to say `delete_trigger`, and that is now forbidden.** Owner, live: *"Delete triggers are the only thing that gives me an approval prompt in automode, this will stall your session untill I get back. Always prevent using them."* A cutover is exactly the unattended moment where that stall costs the most. Disabling achieves the same thing — the old failsafe stops firing — without the prompt, and it is reversible. If the trigger genuinely needs deleting afterwards, say so in your reply and the owner removes it in seconds. **This prompt is not loaded from this repo's hooks, so a satellite session founded from it gets no guard — the correction has to live in the prompt text itself.**
