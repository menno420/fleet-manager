...
# Venture Lab — failsafe cron text (registry copy, prompts v3.7)
...
the money-seat heartbeats (trading status carries the trigger-id records); the weekly grading cron is a BUSINESS cron — rebound per step 4, kept alive; plus ids the heartbeat marks left-for-successor. `list_triggers` is
