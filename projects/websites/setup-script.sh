#!/usr/bin/env bash
# ── REGISTRY NOTE (part 3 of the websites Project package, 2026-07-10) ──────
# CANONICAL LINEAGE PICK: this file is websites docs/project/setup-script.sh
# at origin/main fc8354e, VERBATIM below this note block. The repo carries a
# SECOND setup lineage — scripts/setup-env.sh (gen-2 wind-down script,
# 2026-07-09) + scripts/env-setup.sh (path-compat wrapper for the
# pinned-research archetype hook path) — flagged by inventory-lanes §1
# ("dual setup-script lineage… which field does the console actually hold?").
# docs/project/setup-script.sh is canonical because it is the newer,
# package-homed text (docs/project/README.md names it the console paste) and
# carries the capability-probe block (signing / ls-remote / dry-run push /
# API gate) written from the 2026-07-10 16:01Z stranded-session lesson, which
# the scripts/ lineage predates. The scripts/ pair is a RETIREMENT candidate
# FOR THE LANE (not this package): fold anything unique into this file, keep
# scripts/env-setup.sh only if the deployed environment's hook path still
# invokes it, and record the outcome in docs/CAPABILITIES.md. Which text the
# console field actually holds today is UNVERIFIED (owner-paste convention,
# no paste record) — see meta.md. Fleet standard for this lane's environment:
# fleet-manager environments/archetype-pinned-research.sh (websites row).
# ─────────────────────────────────────────────────────────────────────────────
