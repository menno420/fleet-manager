...
> websites Project → Custom Instructions field (8,000-char paste cap; fitted
> texts target ≤7,500 — fm `docs/proposals/instructions/README.md`; this body
> is 6,989 chars). Source of truth is the repo file
> `websites docs/project/project-instructions.md` — this registry copy
> canonicalizes it. **Provenance:** the repo file's body at websites
> origin/main `fc8354e` VERBATIM except one fix: its § "ROUTINE-FIRED SESSION
> protocol" step 1 still said "execute ONE bounded slice, not a marathon" —
> superseded here with the Q-0265 continuous work-loop shape (owner directive
> Q-0265, 2026-07-10, via the part-4 brief §2b rider; the repo's own
> `docs/project/routine-prompt.md` v2 already carries it — only the
> instructions file had drifted; inventory-lanes §1 flagged this exact drift).
> The ladder itself was already loop-ready; only the pacing sentence changed
> (+ "per slice" on the ladder intro so the two sections agree). After this
> registry copy lands, the lane should port the same fix back into
> `docs/project/project-instructions.md`. Last verified against websites
...
You may be a fresh session fired by the wake routine. Then:
1. `git pull`; re-read `control/inbox.md` at HEAD; claim before building
   (`control/README.md`); then WORK IN A LOOP up the ladder below — when a
   slice lands and useful work remains, take the next slice the same
   session; the throttle is removed, not the ceremony (Q-0265, 2026-07-10,
   supersedes v1's "ONE bounded slice"). Backpressure, not the clock, is
   the brake.
...
FIRST rung with work; ship ONE increment per slice:
