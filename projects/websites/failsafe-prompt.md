...
# Websites — failsafe cron text (registry copy, prompts v3.4)
...
<!-- registry-header-end -->
# Websites — failsafe cron (dead-man wake, Q-0265)

- **Routine name:** `Websites failsafe wake`
- **cron:** `45 */2 * * *` — slot per the stagger table
...
the heartbeat's routine block + the account-wide registry searched by name+prompt (record each id before deleting; a BUSINESS wake is rebound per step 4, never just dropped); plus ids the heartbeat marks left-for-successor. `list_triggers` is
