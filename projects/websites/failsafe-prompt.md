...
(STATELESS, D-9) — find old ids in: the heartbeat's routine block + the account-wide registry searched by name+prompt (record each id before deleting; a BUSINESS wake is rebound per step 4); plus ids the heartbeat
