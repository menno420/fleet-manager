...
| [`../owner/`](../owner/README.md) | **CORE** | **The owner's single attention surface** — one generated index (`tools/gen_owner_index.py`) sweeping the sources below into four sections: decisions owed, open questions, documents written for his hand, ideas awaiting triage. Root-level, not under `docs/`, because it is his front door rather than a document class. Never hand-edit the index; add siblings beside it. |
...
| [`activity/`](activity/README.md) | TASK | **What every session did, wherever it ran** — the derived index of session cards across every repository, plus the hand-written lane for work that touches no repository (laptop, ChatGPT, Drive). The answer to "what happened while I was away", and the only place a *local* session's work is visible from a cloud one. |
| [`owner-comments/`](owner-comments/README.md) | TASK | Durable **public** repository-specific owner feedback: exact JSON records, a stable routed index per ESTATE row, a cheap derived root index, and consumed history preserved by move rather than deletion. |
...
| [`traps.md`](traps.md) | **CORE** | The trap register (D3 / roadmap § 5.4) — the estate's recurring **execution** mistakes in TRAP · TRIGGER · WHY · REQUIRED PREVENTION · VERIFY · ORIGIN form. You are not meant to read it at the moment you need it: six of its seven entries are delivered by `doc-routes` at the point of action. CAPABILITIES records what the estate *can do*; this records what it keeps *getting wrong*. |
| [`SKILLS-local.md`](SKILLS-local.md) · [`SKILLS.md`](SKILLS.md) | TASK | The installed skill roster (one line each — the file carries its own count) and the ⚠ re-apply table for kit upgrades. |
...
| [`prompts/`](prompts/README.md) | RECORD ·mixed | The retired seat-prompt corpus — except the live files enumerated in ONE place, the top of [its README](prompts/README.md) (the two ChatGPT instruction sets, the curious-research review prompt, and the 2026-09-02 continuation prompts — six as of 2026-09-02), all **TASK**. This row used to carry its own list and fell behind; it now points. |
