...
|---|---|---|---|---|
| `superbot-plugin-hello` | the plugin-contract hello-world, hash-pinned in superbot-next's lockfile. **The former "never archive" is CORRECTED 2026-08-22** — read directly, the pin is `manifest_hash: sha256:ff75b9eb…`, and a hash is a verification value, not a fetch, so archiving cannot break it; what archiving blocks is *editing* the manifest, which was always a two-repo change (host pin re-land or FAILED_STARTUP). Disposition: **archive, paired with `superbot-next`** | complete-parked | `README.md` | on demand |
