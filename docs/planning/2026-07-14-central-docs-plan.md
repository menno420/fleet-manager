...
| B4 | Intra-lane ledger splits: `docs/PLATFORM-LIMITS.md` vs `docs/CAPABILITIES.md` (venture, idle, pokemon; gba ×3 incl. a third lowercase seed copy). Seat-digest walls render "(no walls recorded)" while the real walls sit in the un-fenced file (sim-lab inconsistency 5, venture inconsistency 4 — the exact surface fm's seat-prompt regen consumes) | per-repo | Lane-side folds ordered via fm inbox ORDERs; venue-tag entries so the digest stops under-reporting; the seat-digest fence becomes the single machine extraction surface fm consumes |
