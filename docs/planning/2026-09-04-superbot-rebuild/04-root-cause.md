...
| § 2.2 · a guard over an empty population | **every gate declares its population and asserts it EQUALS a committed expected set** derived independently of the walker — a floor alone is not enough, since it passes while the population silently shrinks (corrected after external review; [`08-verification.md`](08-verification.md) § 1) | the exact check that would have failed superbot-next's navigation golden on day one, and would still fail it if 64 of 314 panels quietly went missing |
