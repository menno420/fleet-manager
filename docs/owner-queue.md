...
- **`OQ-FM-APPARATUS-SIZING` — right-size fleet-manager's own apparatus (NEXT-TASKS item 3).**
  ...
  - **KEEP `merge-on-green.yml`** — the repo's server-side landing path (verify-then-squash-merge);
    **not** classifier-gated, load-bearing (agent-side merge is classifier-denied, so this IS the
    landing path). Do not touch.
