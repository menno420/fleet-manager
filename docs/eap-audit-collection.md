...
> in its last column — straggler sweep 2026-07-14 ~12:02Z: **8/13
...
| Repo (menno420/) | Audit doc at HEAD? | Landed PR # | Headline numbers | Top ANTHROPIC asks | Walkthrough at HEAD? (12:02Z straggler sweep) |
|---|---|---|---|---|---|
| superbot | **no** (checked 09:52Z, HEAD `e7d4d68`; unchanged at 12:02Z) — hub narrative corpus lives in its `docs/eap/` by design; closeout ORDER 006 dispatched via #2096, MERGED 2026-07-14T09:43:33Z | — | — | — | **no** (404 @ main, 12:02Z) — ordered via #2096/ORDER 006(b), MERGED 09:43:33Z; doc not yet landed |
...
| idea-engine | **YES** (checked 09:52Z) — covers BOTH Ideas-Lab repos (idea-engine + sim-lab) | **#413** (merged 2026-07-14T09:12:06Z, `8162d1e`) | seat total 545 PRs opened / 543 merged · 772 commits · ≈3.6-day window | (1) cross-repo MCP read scope fixed at session creation (§9.2); (2) branch deletion — 389 undeleted merged branches (§9.3); (3) Write-tool REPORT.md refusal (§9.5); (4) trigger tombstones + cross-session binding + filtered trigger list (§9.4/§10.1) | **no** (404 @ main, 12:02Z) |
| sim-lab | covered — seat audit single-homed at idea-engine (`docs/audits/eap-project-audit-2026-07-14.md@8162d1e` "Covers BOTH repos"); no repo-local doc at HEAD (checked 09:52Z) by design | (via idea-engine #413) | see idea-engine row | see idea-engine row | **no** (404 @ main, 12:02Z) |
...
| pokemon-mod-lab | **in flight** — PR **#84** "EAP project audit 2026-07-14 (full seat edition)" OPEN, `mergeable_state: clean`, head `6349c6c` (opened 09:21:36Z; re-verified STILL OPEN + clean 12:02Z, head unchanged); covers BOTH pml + gba; parks green by design — repo has no merge automation; merging it is owner-checklist row 2 | — (parked green on #84) | per PR body: 26-parked-PR famine (pml) + 17.5 h refuse-to-arm epoch (gba), full denial ledger | per PR body: classifier/merge friction, scheduling/wake friction — read the doc on the PR head | **no** (MCP get_file_contents @ refs/heads/main: not found, 12:02Z) |
