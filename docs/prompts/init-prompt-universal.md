...
⚠ NEVER `delete_trigger` (amended 2026-08-09). It is the one call that raises an owner approval prompt in automode and STALLS the session until he is back. To retire a trigger, `update_trigger` it with `enabled: false` — same effect, no prompt, reversible. Permanent removal is the owner's, on request in your reply. This prompt founds sessions in repos that do NOT load fleet-manager's guard hook, so this line is the only protection they get.
