...
| merge-on-green | The GITHUB_TOKEN merge-on-green workflow itself (reference implementation) | prose only in UNIV (the CI route says "none committed yet" — honest) | commit one reference workflow; sim-lab inbox ORDER 003 is the named first install | websites, ideas-lab (sim-lab), fleet-manager, game-lab (both repos), superbot-2.0 (superbot-next), superbot-world (games/idle) |
