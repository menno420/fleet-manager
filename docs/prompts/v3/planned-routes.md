...
|---|---|---|---|---|
| ~~deny-wins / one-attempt, INJECTION GUARD, backpressure / budgets, claim (CLAIMS rider detail), GIT HYGIENE, TOOL FACTS, TRUTH bar (+TIMESTAMPS), Q-0120, ratification park, outbox/Q-0264 CONTROL BUS text~~ **STRUCK 2026-07-15 (v3.7)** | ~~The full rider texts these entries compress~~ RESOLVED: the v3.7 duty-form rewrite (owner mandate ORDER 048) retired the `CORE` alias — every rider keyword now routes to the seat STARTUP (doctrine carried in full, drift-checked) or to `projects/UNIVERSAL.md` v5 (grant v2 = the option-(a) hoist, executed: landing, denial routing, decide-and-flag, and RULE PROVENANCE are grant text now); deny-wins/park-green were REPLACED by denial-routing/land-on-green, so those keywords have no route to home | homed: STARTUP + UNIV (v3.7 PR) | all 9 |
| ~~merge-on-green~~ **STRUCK 2026-07-12 (v3.4 delta 5); remainder updated 2026-07-15 (v3.7)** | ~~The GITHUB_TOKEN merge-on-green workflow itself (reference implementation)~~ LANDED: sim-lab ORDER 003 executed — the workflow is INSTALLED and live (sim-lab #50; self-landings #50–#52); v3.7 makes install-where-missing a standing first-slice duty (grant v2 LAND EVERY PR bullet) | was: prose only in UNIV ("none committed yet") | homed: sim-lab `.github/workflows/` + fleet-manager merge-on-green.yml (both live references); remaining installs are seat work | still landing-workflow-less at 2026-07-15 verify: game-lab (both repos), superbot-world (games) — fleet-manager (merge-on-green.yml, PR #146), superbot-next + idle (enablers) since resolved |
