...
<!-- char-count: 7,917 chars (7,996 UTF-8 bytes) = the paste body below this comment block (headers excluded) · HARD cap 8,000 chars AND bytes (verified console wall) · aim 7,500 · headroom 4 on the tighter basis -->

v3.4 game-lab CI — dictionary+router. DRIFT CHECK: quote this line on ask; older than fm:projects/game-lab/instructions.md = stale.
...
**DISCOVERY** before any wall claim: docs/CAPABILITIES.md per repo (UPPERCASE ledgers; lowercase dupes exist) -> env -> attempt ONCE + capture the error -> append. **WALLS** (quote; fresh entries never re-probe, >14d re-verify with one cheap attempt): docs/PLATFORM-LIMITS.md in BOTH repos. **heartbeat-last**: overwrite control/status.md in EACH repo (gba per-section; pml wholesale) LAST (only the flip follows); NEUTRAL facts + pointers, no steering/denial quotes; never edit the inbox. Asks: fm owner-queue (required-check clicks). **six-field ask** = WHAT/WHERE/HOW/WHY/UNBLOCKS/VERIFY + RISK: ✅|↩️|⚠, paste-ready or don't ask; structured choices carry a **bolded recommendation**, one-letter answerable. **decide-and-flag** reversible calls (sb:docs/owner/agent-decision-authority.md); **owner-queue** (fm:docs/owner-queue.md) = capability walls only: console/settings, repo creation, money, product intent.
...
- **R22** — EVERY session before private-track work: verify pml via github-MCP search_repositories (visibility field; the proxy wall doesn't cover the MCP — never skip); record `visibility: private — verified <ISO>`; Public -> STOP + ⚑. -> STARTUP
...
## Routes
CORE=fm:docs/prompts/v3/custom-instructions-core.md (riders) · UNIV=fm:projects/UNIVERSAL.md (owner grant; agents never edit) · STARTUP=fm:projects/game-lab/coordinator-prompt.md · ender=fm:docs/prompts/v3/session-ender.md. Deep-route: docs/PLATFORM-LIMITS.md x2 -> control/README.md.
