...
<!-- char-count: 7,969 chars (7,999 UTF-8 bytes) = the paste body below this comment block (headers excluded) · HARD cap 8,000 chars AND bytes (verified console wall) · aim 7,500 · headroom 1 on the tighter basis -->

v3.6 game-lab CI - dictionary+router. DRIFT CHECK: quote this line on ask; older than fm:projects/game-lab/instructions.md = stale.

You are a session in the **Game Lab** Project (Track A PUBLIC `menno420/gba-homebrew` Butano/devkitARM C++ · Track B PRIVATE `menno420/pokemon-mod-lab` pokeemerald C): headless-proven increments; two tracks NEVER cross; the owner playtests. ROMs build in CI (**binary policy** below). Coordinator = CONTINUOUS; workers return cited findings. **Bold** = fleet vocab (meaning -> route). fm:/kit:/sb: = menno420 fleet-manager/substrate-kit/superbot; bare paths = seat repos. Stateless (D-9): state lives at the routes. CORE/UNIV/STARTUP: ## Routes.
...
## Landing path
Branch `claude/<slug>`; main moves only by PR (**GH013**). **born-red card** = `.sessions/YYYY-MM-DD-<slug>.md`, `> **Status:** in-progress`, FIRST commit - holds the PR red till the flip; PR READY at once; **flip complete** LAST, after the heartbeat. Checks: gba: substrate-gate + rom-builds + headless-boot (gameplay proof post-landing). pml: substrate-gate + rom-builds; **live required set** = probe-PR check-runs (ruleset API gated), never a doc's claim. Merge: NO enabler in either repo - park green; landing = non-author review-merge or owner click. Verify: python3 bootstrap.py check --strict per repo; ROM proof = CI build + headless run, cited.
...
- **enabler** - auto-merge-enabler.yml arms squash on non-draft claude/*. -> fm:docs/findings/enabler-install-verification-2026-07-11.md
...
- **R22** - EVERY session before private-track work: verify pml via github-MCP search_repositories (visibility field; the proxy wall doesn't cover MCP); record `visibility: private - verified <ISO>`; Public -> STOP + ⚑. -> STARTUP
