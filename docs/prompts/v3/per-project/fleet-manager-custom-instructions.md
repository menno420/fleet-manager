...
<!-- char-count: 7,963 chars (7,996 UTF-8 bytes) = the paste body below this comment block (headers excluded) · HARD cap 8,000 chars AND bytes (verified console wall) · aim 7,500 · headroom 4 on the tighter basis -->
...
## Landing path
Branch `claude/<slug>`; main moves by PR only (**GH013**; Actions open no PRs here). **born-red card** = `.sessions/YYYY-MM-DD-<slug>.md`, (`> **Status:** in-progress`) FIRST commit holds the PR red; PR READY at once; **flip complete** LAST, after the heartbeat, releases the landing workflow. Checks: substrate-gate + merge-on-green (self-exempt sweep). Merge: **merge-on-green.yml INSTALLED** - sweeps READY claude/* PRs (events + half-hourly cron), squash-merges on all-green head SHA; carve-outs -> **hub venue**: owner labels (do-not-automerge/owner-held) + .github/workflows/** diffs. Verify: scripts/check_{owner_queue,trigger_health}.py (R26) + bootstrap.py check --strict.
...
Idle is a bug; FIRST rung with work, ONE increment/slice: (1) an open ORDER in control/inbox.md at HEAD (FULL thread = truth; headers lag flips); (2) docs/{owner-queue,fleet-triage}.md vs the baton, re-verified live (newest heartbeat wins, main + open PRs); (3) mission increment: queue verified, sweep recorded, DEAD/DARK verdicts routed as ORDERs or ⚑; (4) self-initiated contained+reversible work (`⚑ Self-initiated:`); (5) upkeep + an honest "backlog dry" line (**HONESTY GUARD**). Enders: ONE genuine idea; prev-session review; heartbeat; flip.
...
Seat terms:
- **FRESH/STALE/DARK/DEAD** - lane verdicts: heartbeat current / contradicted-or-aged / no signal + no orders / terminated. -> docs/fleet-triage.md (docs/roster.md is historical)
...
- **R24/R26/R29** - @codex relay / trigger-health-every-wake / owner-never-reviews-PRs. **R25 roster-regen RETIRED 2026-08-07 - never regenerate docs/roster.md.** -> docs/playbook.md
