...
**Closed 2026-08-11 (fm #849):** resolved the way the refuter framed it — by saying which side is over-broad. The parent banner (and its copy in `control/outbox.md`) now carries a claims carve-out: `claims/` is NOT retired — the kit wires it (`claims_dir`, the advisory claims check, `session-close`'s claim step) — while the claims README gains a contested-status note naming its own retired parts (the fast-lane and heartbeat references). Prove: `grep -c 'is NOT retired' control/README.md` → **1** · `grep -c 'Contested-status note, 2026-08-11' control/claims/README.md` → **1** · `grep -c 'is NOT retired' control/outbox.md` → **1**.
...
**Closed 2026-08-11 (fm #849):** `control/status.md` removed from `heartbeat_files` in substrate.config.json — the seat closed 2026-07-21, so the [status-stale] advisory could only ever instruct writing a fresh liveness date onto a closed seat's record (which is exactly what the automated regen did once). The gate runs clean after the change, no status-stale advisory, exit unchanged. Prove: `python3 -c "import json;print(json.load(open('substrate.config.json'))['heartbeat_files'])"` → **[]**.
...
**Closed 2026-08-11 (fm #849):** the Ownership model section is rewritten to the current truth with the seat-era model quoted as a dated retraction; the § New areas manager clause is corrected in the same pass; the source slot is fixed (D43) so regens agree; and the certifying router line that vouched for this file is scoped under D48. Prove: `grep -c 'no manager seat' docs/ownership.md` → **2** · `sed -n '/## Ownership model/,/## Ownership table/p' docs/ownership.md | grep -c '^One writer per file'` → **0**.
...
**Closed 2026-08-11 (fm #849):** §0 now names **two** private repos, re-measured today via `/user/repos?affiliation=owner` (exactly pokemon-mod-lab and shiftlife of 24 owned), notes the 2026-08-08 correction block's inheritance, and corrects the dead roster pointer in the same bullet. Prove: `grep -c 'two PRIVATE repos' docs/reading-path.md` → **1**.
