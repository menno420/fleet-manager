...
| (d) walls & conditionals: `CAPABILITIES.md` walls section · `README.md` regen line · `.session-journal.md` P10 · `auto-merge-guards.md` · `recipes/README.md` · `fleet-repos.txt` · `house-style.md` cell · `ai-project-workflow.md` slot · `telemetry/README.md` backfill guarantee · `repo-navigation-map.md` placeholder table | targeted corrections in each file's own grammar (the CAPABILITIES append-log's 2026-07-18 correction entry is the model); the roster fix adds the five missing adopters; the telemetry fix era-closes the backfill sentence; the map fix fills the rows per its own rule | standing walls contradicting the estate's verified capability matrix, in the false-walls checker's own repo; the roster is live machinery input, so its gap ships into every future regen; a false operational guarantee and an empty router are the same wrong-action class |
