...
| **Every `carry` row's source SHA** | A carry copies bytes. If the source moved, the copy is a stale fork of a live document — the exact failure the fresh start exists to end. **Mechanical since fm #1036:** `python3 tools/estate_baseline/row_delta.py --manifest … --classification … --out …` classifies every row (`SOURCE_UNCHANGED` / `SOURCE_MOVED` / `SOURCE_GONE` / `UNCHECKABLE:<reason>`), and its snapshot at `data/…/row-delta.tsv` is the state at 2026-09-04T18:38Z — [the follow-on finding](2026-09-04-estate-seed-prerequisites.md) reads it. |
