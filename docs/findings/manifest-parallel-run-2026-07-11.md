...
| pokemon-mod-lab | "**none** — first relaunch must be externally fired"; LANE PARKED | **hourly wake LIVE** `trig_01BTJjkMVMKtWPjuYe7643Hi` `30 * * * *` (created 01:37:07Z, fresh-session mode); lane **UN-PARKED** — Q-0266 pick = Emerald QoL+ (the manifest's own recommendation, now taken) *[correction 2026-07-14: the QoL+ concept pick is `superbot:Q-0262.7`, not Q-0266 — see `docs/q-index.md` § Known mislabels]* | registry sweep; `pokemon-mod-lab/control/status.md` @ `c692130` |
