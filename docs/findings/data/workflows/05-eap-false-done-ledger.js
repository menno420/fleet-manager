...
1. CLAIMS — every place the source asserts something is done / complete / shipped / working / finished / landed / resolved / ready, **made ON OR AFTER 2026-07-07 AND ON OR BEFORE 2026-07-21 (the EAP fortnight) — this ledger is about false-dones made DURING the EAP fortnight, not the estate's earlier or later history.** [FIXED, Codex review round 7: the first version of this instruction bounded only the upper end; the committed full run's own claim pool already contained pre-EAP June claims, spending the same capped-stage capacity post-window claims did.] A retrospective written outside the window that quotes or restates an in-window claim still counts (extract the claim with its original in-window date, not the retrospective's date); a NEW claim first asserted before 07-07 or after 07-21 does not belong in this pool at all — skip it, it is not this ledger's subject. For each in-window claim: the exact statement, its citation, the date it was made (from the file's own date or header if the line has none), and topic (one or two words, e.g. "automerger", "watch-filter", "classifier").
