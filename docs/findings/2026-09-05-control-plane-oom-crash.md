...
came true 15 days later. Memory went from a **flat 0.155 GB baseline to 1.93 GB
in under four hours** and stayed pinned at the container ceiling for 19 hours
with CPU welded to a full core; the app is so starved that `/healthz`, which
does no I/O at all, **took 11.97 s** on a live probe. **No other Railway
...
0.154–0.155 GB for twelve hours**:

| Time (UTC) | Memory | What it means |
|---|---|---|
| 09-03 21:00 → 09-04 08:00 | **0.154–0.155 GB** | healthy baseline, twelve hours flat |
...
continuously** — then collapsed to 0.14 at the kill. The peak before each kill is
~1.93 GB; the container limit is therefore ~2 GB (`REASONED` — I read the
observed peak, not a configured limit field).
...
A pure in-memory route taking twelve seconds is **event-loop starvation**, and it
means the Railway healthcheck on `/healthz` is at the edge of failing too.
...
2. **Every request rebuilds everything.** `owner_queue.overview()` is **not
   ...
   GitHub HTTP responses are TTL-cached (`app/github.py`) — and that cache is
   **not** the leak: the live `/healthz` reports **170 entries**. So per request
...
3. **Nothing bounds the concurrency.** The Dockerfile CMD is a bare
   ...
   `--limit-concurrency`, no request timeout**. At the measured 40.6 req/min and
   53 s service time, Little's law puts ~**36 requests in flight at once**
   (`REASONED`), each holding its own full copy of that object graph. That
   product is the 1.9 GB. It also explains the plateau: CPython returns freed
   arenas to the OS only grudgingly, so RSS stayed at the high-water mark for 19
   hours rather than falling back.
...
- **B (complementary, cheap, and worth doing regardless):** bound the blast
  radius — `--limit-concurrency` on the uvicorn command and a memoized
  `overview()` behind the existing TTL. This is the durable fix: it stops *any*
  future traffic spike on *any* faceted route from converting into an OOM, where
  A only moves this particular crawler onto the next page. **A stops today's
  bleeding; B is why it happened at all.**
- **Rejected, and stays rejected:** an IP-range block (breaks real link
  unfurling, § 3) and relying on robots.txt (measured ignored — 0 fetches in
  3,001 requests on 2026-08-20, and the crawl continues today).
