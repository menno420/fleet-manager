# fleet-manager — the shipped log and the seat-era record

> **Status:** `historical`
>
> **What this is:** everything that was moved off
> [`current-state.md`](current-state.md) on **2026-08-22** under **OD-17**
> (*"cleaning up excessive repos and documentation… while still making it easy
> for an agent to understand work"*). It is the hub's **merged-work log** plus
> the **seat-era sections its own header already marked as preserved, not
> current**.
>
> **Why it moved.** `current-state.md` is the second file of the repository's
> three-file front door and its header states what it is for: it *"carries live
> hub state, not the program history."* It had grown to **6,212 words**, of
> which **4,383 were this merge log** — 70% of a mandatory read, describing work
> already merged. The boot read set sat at **7000 of 7000 words, zero headroom**,
> and the kit's own `orientation-headroom` advisory was firing. Cutting by tier
> rather than by volume ([`MAP.md`](MAP.md): CORE / TASK / RECORD) means this
...
> `RECORD` tier. For what is true now read
> [`current-state.md`](current-state.md); for what to do next read the
> [consolidation program](planning/2026-07-26-consolidation-program.md)'s NOW
> pointer.

## Recently shipped — the older entries (newest first)

These continue directly below the eight most recent entries, which stay in
`current-state.md`. Together the two lists are the complete log.
...
## Superseded sections lifted from `current-state.md`

Both were already badged *preserved, superseded* in place; they are kept
here so the live file carries only what is current.
