# fleet-manager — routines & wake-chain doctrine

> **Status:** `binding`
>
> **Era note, 2026-08-11 (audit D51):** the *method* below — probe-not-record,
> registry-to-exhaustion before absence claims, the wedge and manual-fire
> signatures, sequential pacing — remains sound. Two things changed after it
> was written and the body must not be followed against them: **(1) the live
> rule is [D‑0015]** (docs/decisions.md, 2026-08-09) — a session never calls
> `delete_trigger`; `.claude/hooks/trigger_tools_guard.py` DENIES the call,
> and the emergency stop is `update_trigger` with `enabled: false`. Where the
> body treats deletion as ordinary recorded work, read "disable" — deleting is
> the owner's, from the console. **(2) The seats are gone** (program closed
> 2026-07-21): the § "Seat wake discipline" end-of-turn invariant — one tick
> always armed, `control/status.md` re-stamped — describes the retired
> apparatus; do not re-stamp the closed seat's heartbeat.
>
