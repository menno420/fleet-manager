...
> **Status:** `living-ledger` · true as of **2026-08-21** (thread state
> refreshed by the fleet review, fm #878; the § "How much of the repo this was
> built from" section below remains the 2026-08-08 construction record)
>
...
enforces pace. `MEASURED` 2026-08-21 against the GitHub API: `menno420/spider-swing`,
**public**, default branch `main`, created 2026-07-28, last pushed 2026-08-17.
`spider-swing` is the **repo** name and internal codename; **Slingy
Spider** is the publishing name, decided 2026-08-05 — and since 2026-08-05 a
**signed build (version code 64) sits on Google Play's internal testing
track** under `com.menno420.slingyspider` (the repo's own ledger, § "What
...
Where it stands: build `0.45.0-run-feedback` (vc66) is current
(`project.godot`, `MEASURED` 2026-08-21). Continuous drive is
...
controls as a stopgap. PR #176 (2026-08-13) paired **first-session
comprehension feedback** to run records (schema-2 ledger) — the tuning
instrument grew; nothing has yet used it for a tuning pass.
...
### Thread: Google Play release — **active, owner-gated**, updated 2026-08-21

Where it stands: **further along than this thread said until 2026-08-21.** The
app ID is chosen and burned in and the upload key exists — the owner published
a **signed AAB (version code 64)** under `com.menno420.slingyspider` on the
**internal testing** track on 2026-08-05 (spider-swing `docs/current-state.md`
§ "What measurement has settled"; `OQ-PLAY-APP-ID` and `OQ-PLAY-UPLOAD-KEY`
are resolved in [`../../owner-queue.md`](../../owner-queue.md)). Internal
testing buys **zero** progress on the 12-tester clock — the closed test is the
track that counts, and it cannot start until the listing is complete.

What is genuinely waiting on the owner now, with stable slugs in
[`../../owner-queue.md`](../../owner-queue.md):
- `OQ-PLAY-LISTING` — **the critical path**: store listing, App content and
  pricing gate the closed track. Copy is drafted in spider-swing
  `docs/product/play-store-listing.md`; the **screenshots must be real
  capture** (the `android-debug` workflow builds an installable APK on every
  push to `main`), because generated imagery invents UI and physics.
- `OQ-PLAY-PRIVACY-POLICY` — a live public URL, mandatory even for
  "collects nothing".
- The closed test itself — 12 testers × 14 continuous days, self-serve via an
  anyone-can-join Google Group (the verified route; see the queue entry).

The name thread is **settled for launch** (spider-swing
`docs/product/name-status.md`, PR #171): a live SLINGY mark exists in adjacent
classes — launch as planned, do not register yet, consult a Benelux attorney
if traction.
...
Next step for a session: produce **real device captures** for the listing —
the one agent-executable part of the critical path.

### Thread: run evidence — **closed 2026-08-06** (spider-swing #172, #173; extended by #176, 2026-08-13)

Closed deliberately and recorded because it is what makes the difficulty thread
measurable — and extended after close by PR #176, which took the ledger to
schema-2 and paired first-session comprehension feedback to run records (the
core-feel thread above carries that). As shipped: a schema-1 `RunRecord` +
`RunRecordLedger` (newest 100 full
