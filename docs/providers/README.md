...
| [`gemini-notebook.md`](gemini-notebook.md) | **A different surface from `gemini.md`, and conflating the two is the mistake it exists to stop.** Gemini Notebook = NotebookLM renamed: the source cap and which half of it is confirmed, what it refuses to ingest, the partition-never-concatenate rule, and the two build findings that cost a session to learn |
