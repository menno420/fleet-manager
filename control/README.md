...
> The `control/` message-bus (`inbox.md`, `outbox.md`, `status.md`) and the
...
In THIS repo the roles invert — `control/inbox.md` is written by the **OWNER** (orders to the
manager); `control/status.md` is written by the **manager** (its heartbeat, refreshed each
working session).
