...
updated: 2026-07-12T09:05Z — CONTINUOUS OPERATION (successor coordinator session_01FMJoC5uC6WSUTosceTGcmo live; predecessor archived 21:50Z). Registry-sync lane worker (owner-directed) re-stamped this heartbeat with the projects/-to-v3.2 sync record below.
...
- **Registry sync — projects/ seat copies regenerated from prompts v3.2 (owner-directed 2026-07-12) — fm PR #110:** all 8 standing-seat packages (fleet-manager, superbot-2.0, websites, self-improvement, superbot-world, game-lab, ideas-lab, venture-lab) × 3 files (coordinator-prompt / instructions / failsafe-prompt) regenerated from `docs/prompts/v3` @ `6391b2f` (v3.2 stateless generation): coordinator-prompt = the seat's startup B file verbatim; instructions = the assembled CI paste per the v3.2 README recipe (7,945–8,000 chars per seat, all ≤ the 8,000-char cap; the fleet budget basis is characters — raw UTF-8 bytes run 8,052–8,111); failsafe-prompt = A step-3a wake text + D-7 stagger-table cron (the table supersedes pre-rebuild crons previously recorded in the superbot-world/game-lab/websites copies). Every copy carries a GENERATED-COPY provenance header — source of truth = `docs/prompts/v3`. Drift guard shipped: `docs/prompts/v3/tools/regen_b_files.py --check-registry` (exit 0 on the synced tree; `--write-registry` regenerates). `projects/README.md`: slice-3 note + paste-wave supersession pointer (MATRIX left historical). Follow-up: CI wiring for `--check-registry` (substrate-gate.yml is kit-owned; the roster-freshness.yml sibling-workflow pattern is the path).
